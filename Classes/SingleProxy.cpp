...
//初始化为主机是否成功
bool SingleProxy::StaticInitAsMasterPeer(uint16_t inPort, const string& inName)
{
    sInstance = std::make_unique< SingleProxy >();//创建指针

    return sInstance->InitAsMasterPeer(inPort, inName);
}
//初始化为分机是否成功
bool SingleProxy::StaticInitAsPeer(const SocketAddress& inMPAddress, const string& inName)
{
    sInstance = std::make_unique< SingleProxy >();
...
bool SingleProxy::InitAsMasterPeer(uint16_t inPort, const string& inName)
{
...
bool SingleProxy::InitAsPeer(const SocketAddress& inMPAddress, const string& inName)
{
    //if (!InitSocket(0))
    //{
        //return false;
...
/*
bool SingleProxy::InitSocket(uint16_t inPort)
{
    mSocket = SocketUtil::CreateUDPSocket(INET);
    SocketAddress ownAddress(INADDR_ANY, inPort);
    mSocket->Bind(ownAddress);

    LOG_SU("Initializing SingleProxy at port %d", inPort);
...
void SingleProxy::ProcessIncomingPackets()
{
...
void SingleProxy::SendOutgoingPackets()
{
    switch (mState)
    {
        //case NMS_Hello:
...
/*
void SingleProxy::UpdateSayingHello(bool inForce)
{
...
void SingleProxy::SendHelloPacket()
{
...
void SingleProxy::UpdateStarting()
{
...
void SingleProxy::UpdateSendTurnPacket()
{
    ...
    if (mSubTurnNumber == kSubTurnsPerTurn)
    {
        ...
        //save our turn data for turn + 2 保存
...
/*
void SingleProxy::TryAdvanceTurn()
{
    ...
    if (mTurnData[mTurnNumber + 1].size() == mPlayerCount)
    {
        if (mState == NMS_Delay)
        {
            //throw away any input accrued during delay在延迟时的所有操作清空
            CommandList::sInstance->Clear();
            mState = NMS_Playing;
...
void SingleProxy::ProcessPacket(InputMemoryBitStream& inInputStream, const SocketAddress& inFromAddress)
{
    switch (mState)
    {
        //case NMS_Hello:
...
/*
void SingleProxy::ProcessPacketsHello(InputMemoryBitStream& inInputStream, const SocketAddress& inFromAddress)
{
...
void SingleProxy::HandleNotMPPacket(InputMemoryBitStream& inInputStream)
{
...
void SingleProxy::HandleWelcomePacket(InputMemoryBitStream& inInputStream)
{
...
void SingleProxy::ProcessPacketsLobby(InputMemoryBitStream& inInputStream, const SocketAddress& inFromAddress)
{
...
/*
void SingleProxy::HandleHelloPacket(InputMemoryBitStream& inInputStream, const SocketAddress& inFromAddress)
{
...
void SingleProxy::HandleIntroPacket(InputMemoryBitStream& inInputStream, const SocketAddress& inFromAddress)
{
...
void SingleProxy::HandleStartPacket(InputMemoryBitStream& inInputStream, const SocketAddress& inFromAddress)
{
...
void SingleProxy::ProcessPacketsPlaying(InputMemoryBitStream& inInputStream, const SocketAddress& inFromAddress)
{
    uint32_t	packetType;
    inInputStream.Read(packetType);
...
void SingleProxy::HandleTurnPacket(InputMemoryBitStream& inInputStream, const SocketAddress& inFromAddress)
{
    /*
    if (mSocketToPlayerMap.find(inFromAddress) != mSocketToPlayerMap.end())
    {
...
/*
void SingleProxy::ProcessPacketsDelay(InputMemoryBitStream& inInputStream, const SocketAddress& inFromAddress)
{
...
void SingleProxy::HandleConnectionReset(const SocketAddress& inFromAddress)
{
...
void SingleProxy::ReadIncomingPacketsIntoQueue()
{
...
void SingleProxy::ProcessQueuedPackets()
{
...
void SingleProxy::UpdateHighestPlayerId(uint32_t inId)
{
    mHighestPlayerId = std::max(mHighestPlayerId, inId);
}

void SingleProxy::EnterPlayingState()
{
    mState = SG_Playing;
...
/*
bool SingleProxy::CheckSync(IntToTurnDataMap& inTurnMap)
{
...
*/
void SingleProxy::SendPacket(const OutputMemoryBitStream& inOutputStream, const SocketAddress& inToAddress)
{
...
void SingleProxy::TryStartGame()
{
    if (mState < SG_Starting && IsMasterPeer())
    {
...
void SingleProxy::UpdateBytesSentLastFrame()
{
...
/*
SingleProxy::ReceivedPacket::ReceivedPacket(float inReceivedTime, InputMemoryBitStream& ioInputMemoryBitStream, const SocketAddress& inFromAddress) :
...
//可能的重写：游戏对象id
EntityPtr SingleProxy::GetGameObject(uint32_t inNetworkId) const
{
...
EntityPtr SingleProxy::RegisterAndReturn(Entity* inGameObject)
{
...
void SingleProxy::UnregisterGameObject(Entity* inGameObject)
{
...
void SingleProxy::AddToNetworkIdToGameObjectMap(EntityPtr inGameObject)
{
    mNetworkIdToGameObjectMap[inGameObject->GetNetworkId()] = inGameObject;
}

void SingleProxy::RemoveFromNetworkIdToGameObjectMap(EntityPtr inGameObject)
{
    mNetworkIdToGameObjectMap.erase(inGameObject->GetNetworkId());
}

void SingleProxy::RegisterGameObject(EntityPtr inGameObject)
{
    //给新实体分配新网络id
...
uint32_t SingleProxy::GetNewNetworkId()
{
...
uint32_t SingleProxy::ComputeGlobalCRC()
{
